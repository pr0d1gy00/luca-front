    ...
    <div>
      <input
        ...
        className={[
          ...
          "focus:outline-none focus:ring-2 focus:ring-[#23dce1]/20 focus:border-[#23dce1]",
...
function StatusBadge({
  ...
  if (status === "ACTIVE" && verified) {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-100 text-xs font-semibold text-emerald-600">
  ...
  if (!verified) {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 border border-amber-100 text-xs font-semibold text-amber-600">
        <ShieldCheck className="w-3.5 h-3.5" />
...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-slate-100 last:border-0">
      ...
      <span className="text-sm text-slate-700 font-medium text-right truncate max-w-[60%]">
        {value || "—"}
      </span>
    </div>
  );
      ...
      <div className="flex justify-end">
        <button
          ...
          className={[
            ...
            isDirty && !loading
              ? "bg-[#23dce1] hover:bg-[#1fc8cd] text-white shadow-sm hover:shadow-md"
...
function UserProfileFormInner({ initial }: { initial: UserProfile }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-7">
      ...
      {/* Info readonly */}
      <div className="rounded-xl bg-slate-50 border border-slate-100 p-4 space-y-0">
      ...
      {initial.provider_profile && (
        <div className="rounded-xl bg-slate-50 border border-slate-100 p-4">
          <p className="text-xs font-semibold text-slate-400 mb-2 pb-2 border-b border-slate-100">
            Datos comerciales
          </p>
