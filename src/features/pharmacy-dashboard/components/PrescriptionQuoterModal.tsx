...
export function PrescriptionQuoterModal({
  ...
  >(() => {
    const initialItems =
      quoteRequest?.prescription?.items?.map((item, idx) => ({
        tempId: `presc-${idx}`,
        prescription_item_id: item.id,
        originalName: item.medication?.name || `Medicamento #${idx + 1}`,
        custom_product_name: item.medication?.name || "",
  ...
  const calculateTotalUSD = () => {
    return items.reduce(
      (acc, item) => acc + (item.prices_manual?.USD || 0) * item.quantity,
      0,
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-3xl shadow-none my-8 overflow-hidden space-y-0">
        {/* Header */}
            ...
            <div className="space-y-4">
              {items.map((item, index) => {
                ...
                return (
                  <div
                    key={item.tempId}
                    className={`p-4 rounded-xl border transition-colors shadow-none space-y-3 ${
                      isAvailable
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border shadow-none ${
                            isAvailable
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className={`w-full h-10 px-3 rounded-xl text-xs font-bold border transition-colors ${
