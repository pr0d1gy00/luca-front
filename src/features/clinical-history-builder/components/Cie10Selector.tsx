      ...
      {selectedEntry && !isOpen ? (
        <div
          className={cn(
            ...
            disabled
              ? "bg-pharmako-care-light/20 border-pharmako-care/10"
              : "bg-pharmako-care-light border-pharmako-care/40",
          )}
        >
            ...
            <div className="flex items-center gap-2">
              <span
                className={cn(
                  ...
                  disabled
                    ? "text-pharmako-care bg-pharmako-care-light/40 border-pharmako-care/15"
                    : "text-pharmako-care bg-pharmako-care-light border-pharmako-care/40",
                )}
              >
                {selectedEntry.code}
              </span>
              <span
                className={cn(
                  "text-sm truncate",
                  disabled ? "text-pharmako-care/80" : "text-pharmako-care",
                )}
              >
          ...
          {!disabled && (
            <button
              type="button"
              onClick={handleClear}
              className="p-1 rounded hover:bg-pharmako-care-light text-pharmako-care transition-colors flex-shrink-0"
            >
        ...
        <div className="relative">
          ...
          <input
            type="text"
            value={disabled ? value || "" : query}
            onChange={(e) => !disabled && handleSearch(e.target.value)}
            onFocus={() => !disabled && query && setResults(searchCie10(query))}
            onKeyDown={(e) => !disabled && handleKeyDown(e)}
            placeholder={
              disabled
                ? "Sin registrar"
                : (element.placeholder ??
                  "Buscar código o descripción CIE-10...")
            }
            readOnly={disabled}
            className={cn(
              "w-full pl-9 pr-8 py-2 rounded-xl border text-sm transition-shadow",
              disabled
                ? "bg-slate-50/50 border-slate-200 text-slate-500 cursor-default shadow-none focus:outline-none"
          ...
          {results.map((entry, idx) => (
            <li key={entry.code}>
              ...
              >
                ...
                {value === entry.code && (
                  <Check className="w-4 h-4 text-pharmako-care flex-shrink-0" />
                )}
              </button>
            </li>
          ))}
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white rounded-xl border border-slate-100 shadow-lg px-4 py-6 text-center">
          ...
          {element.allowFreeText && (
            <button
              ...
              className="mt-2 text-xs text-pharmako-care hover:text-pharmako-care-hover underline"
            >
