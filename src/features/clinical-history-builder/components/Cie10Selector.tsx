...
const CIE10_DATASET: Cie10Entry[] = [
  ...
  { code: 'A09', description: 'Diarrea y gastroenteritis de origen infeccioso' },
  ...
  { code: 'F10', description: 'Trastornos mentales y de comportamiento debido al uso de alcohol' },
  ...
  { code: 'J06', description: 'Infecciones agudas de las vías aéreas superiores múltiples' },
  ...
  { code: 'O10', description: 'Hipertensión preexistente complicando el embarazo' },
  ...
  { code: 'Z00', description: 'Examen e investigación normal de pacientes sin diagnóstico' },
  { code: 'Z23', description: 'Necesidad de inmunización contra enfermedades infecciosas' },
  { code: 'Z34', description: 'Supervisión de embarazo normal' },
  { code: 'Z38', description: 'Recén nacido vivo según lugar de nacimiento' },
  { code: 'Z70', description: 'Asesoramiento relacionados con la salud sexual' },
  { code: 'Z76', description: 'Personas en contacto con los servicios de salud en otras circunstancias' },
];
...
export function Cie10Selector({ element, value, onChange }: Cie10SelectorProps) {
      ...
      {selectedEntry && !isOpen ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-teal-50 border border-teal-100">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-teal-600 bg-teal-100 px-1.5 py-0.5 rounded">
                {selectedEntry.code}
              </span>
          ...
          <button
            type="button"
            onClick={handleClear}
            className="p-1 rounded hover:bg-teal-100 text-teal-500 transition-colors flex-shrink-0"
          >
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={element.placeholder ?? 'Buscar código o descripción CIE-10...'}
            className={cn(
              'w-full pl-9 pr-8 py-2 rounded-xl border border-slate-200 text-sm text-slate-900',
              'placeholder-slate-400 focus:outline-none focus:ring-2',
              'focus:ring-teal-500/20 focus:border-teal-400 transition-shadow',
            )}
          />
          {query && (
            <button
