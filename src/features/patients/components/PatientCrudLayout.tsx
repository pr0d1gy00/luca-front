...
const MOCK_PATIENTS: Patient[] = [
  ...
  },
  {
    firstName: "Carlos",
    lastName: "Rodríguez",
    documentId: "08.765.432",
    birthDate: new Date("1954-03-15"),
    biologicalSex: "MALE",
    ...
    emergencyContactName: "", // Alerta: Ficha sin contacto de emergencia
    emergencyContactPhone: "",
  },
  {
    firstName: "María Luz",
    lastName: "Ortega",
    documentId: "48.123.456",
    birthDate: new Date("2015-08-05"),
    biologicalSex: "FEMALE",
  ...
  },
  {
    firstName: "Sofía",
    lastName: "Martínez",
    documentId: "10.234.567",
    birthDate: new Date("1961-11-30"),
    biologicalSex: "FEMALE",
...
export function PatientCrudLayout({
  ...
  const handleConfirmDelete = () => {
    if (deleteConfirm) {
      setList((prev) => prev.filter((p) => p.documentId !== deleteConfirm));
  ...
  const handleSubmit = (data: Patient) => {
    if (mode === "create") {
      ...
      setList((prev) =>
        prev.map((p) =>
          p.documentId === selectedPatient.documentId ? data : p,
        ),
      );
    }
    setMode(null);
  ...
  const allergicPatients = list.filter((p) => p.allergies.length > 0).length;
  ...
  const complexPatients = list.filter(
    (p) => p.chronicConditions.length >= 2,
  ).length;
  ...
  const malePatients = list.filter((p) => p.biologicalSex === "MALE").length;
  const femalePatients = list.filter(
  ...
  const allAllergies = list.flatMap((p) => p.allergies);
  ...
  const allChronic = list.flatMap((p) => p.chronicConditions);
                ...
                {complexPatients > 0 ? (
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                      {list
                        .filter((p) => p.chronicConditions.length >= 2)
                        .map((p, idx) => (
                          <div
                            key={idx}
                  ...
                  <div className="bg-white rounded-xl p-4 flex items-start gap-3.5 transition-all hover:border-slate-350 hover:bg-pharmako-care-light/5">
                    ...
                    <div className="min-w-0">
                      ...
                      <span
                        ...
                        title={selectedPatient.documentId}
                      >
            ...
            <div className="mt-8 space-y-6">
              ...
              <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <span className="text-[11px] text-slate-400 font-medium block mt-0.5">
                    DNI: {selectedPatient.documentId} | Edad:{" "}
                    {calculateAge(new Date(selectedPatient.birthDate))} años
                  </span>
                </div>
              ...
              <div className="pt-2">
                <ClinicalHistoryTimeline
                  entries={
                    MOCK_CLINICAL_HISTORY[selectedPatient.documentId] || []
                  }
                />
              </div>
