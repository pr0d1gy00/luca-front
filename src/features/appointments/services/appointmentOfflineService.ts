...
export const appointmentOfflineService = {
  ...
  create: async (data: CreateAppointmentDTO): Promise<Appointment> => {
    try {
      ...
      console.warn(
        "[appointmentOfflineService] IndexedDB unavailable or closed, throwing to trigger API fallback:",
        err,
      );
      throw err;
    }
  ...
  /**
   * Save appointment from API response directly into local IndexedDB as synced
   */
  saveLocalSynced: async (data: Record<string, unknown>): Promise<void> => {
    try {
      if (!db.isOpen()) {
        await db.open();
      }
      ...
      const appointment: Appointment = {
        ...
        patientUuid: String(data.patientUuid ?? data.patient_uuid ?? ""),
        doctorUuid: String(data.doctorUuid ?? data.doctor_uuid ?? ""),
        clinicBranchUuid: String(data.clinicBranchUuid ?? data.clinic_branch_uuid ?? ""),
        date: String(data.date ?? ""),
        time: String(data.time ?? ""),
        slotTime: (data.slotTime ?? data.slot_time ?? null) as string | null,
        type: (data.type ?? "IN_PERSON") as Appointment["type"],
      ...
      console.warn(
        "[appointmentOfflineService] Error saving local synced appointment:",
        err,
      );
    }
  },
