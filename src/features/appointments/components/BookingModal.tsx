...
export function BookingModal({
	...
	const doctorBranches = useMemo(() => {
		...
		doctor.clinics.forEach((clinic) => {
			clinic.branches.forEach((branch) => {
				...
				list.push({
					id: branch.id,
					name: cityName ? `${clinic.name} - ${branch.name} (${cityName})` : `${clinic.name} - ${branch.name}`,
	...
	const selectedBranchOption = useMemo(() => {
		return branchOptions.find((opt) => opt.value === String(selectedBranchId)) || null;
				...
				<DialogContent className="rounded-2xl bg-white border border-pharmako-border max-w-lg sm:max-w-xl md:max-w-2xl shadow-xl p-6 lg:p-8">
					<div className="flex flex-col items-center py-6 text-center">
						...
						<p className="text-sm text-pharmako-text-secondary mb-6">
							Tu cita con el Dr. {doctor.fullName} ha sido registrada. Te
						...
						<Button
							onClick={handleClose}
							className="w-full h-11 rounded-xl bg-pharmako-care hover:bg-pharmako-care/90 text-white font-semibold transition-all duration-200"
						>
							Cerrar
						</Button>
					</div>
				...
				<div className="flex flex-col gap-6 py-4">
					...
					<div className="flex items-center gap-3.5 p-4 border border-pharmako-border-soft rounded-xl bg-slate-50/50">
						...
						<div className="min-w-0">
							...
							<p className="text-xs text-pharmako-text-secondary font-medium">{doctor.specialtyName}</p>
						</div>
					</div>
					...
					{selectedDate && availability && availability.exception && (
						<div className={`flex items-start gap-3 p-3.5 border rounded-xl ${availability.isAvailable
							? "bg-blue-50/50 border-blue-100 text-blue-800"
							: "bg-amber-50/50 border-amber-100 text-amber-800"
							}`}>
							<AlertCircle className={`size-5 shrink-0 mt-0.5 ${availability.isAvailable ? "text-blue-500" : "text-amber-500"
								}`} />
							<div>
								...
								{availability.exception.reason && (
									<p className={`text-xs mt-0.5 ${availability.isAvailable ? "text-blue-600/90" : "text-amber-600/90"
										}`}>
										{availability.exception.reason}
									</p>
								)}
								...
								<div className="grid grid-cols-4 gap-2">
									{availability.slots.map((slot) => (
										<button
											...
											className={`h-9 rounded-xl text-sm font-semibold transition-all ${selectedSlot?.time === slot.time
					...
					<div className="flex flex-col gap-2">
						...
						>
							Notas adicionales{" "}
							<span className="text-pharmako-text-muted/60 font-normal lowercase">(opcional)</span>
						</label>
						<textarea
							id="notes"
							rows={2}
