...
export function AppointmentCrudLayout({
	appointments,
	doctors,
}: AppointmentCrudLayoutProps) {
		...
		<>
					...
					<SheetHeader className="pb-5 border-b border-slate-100">
						...
						<SheetDescription>
							{mode === "create"
								...
								: `Editando cita con ${doctors.find((d) => d.uuid === selectedAppointment?.doctorUuid)?.fullName ?? ""}`}
						</SheetDescription>
					</SheetHeader>
						...
						<div className="mt-6 space-y-4">
								...
								<div className="flex items-center gap-3">
									<div className="w-10 h-10 rounded-full bg-luca-primary/20 flex items-center justify-center">
										<span className="text-sm font-medium text-luca-primary">
											{doctors
												.find((d) => d.uuid === selectedAppointment.doctorUuid)
												?.fullName.charAt(0) ?? "?"}
										</span>
									</div>
									<div>
										<p className="font-medium text-luca-muted-dark">
											{
												doctors.find(
													(d) => d.uuid === selectedAppointment.doctorUuid,
												)?.fullName
											}
										</p>
								...
								<dl className="space-y-3 text-sm">
									<div className="flex items-center gap-3">
										<div className="w-8 h-8 rounded-lg bg-luca-surface-light flex items-center justify-center">
											<svg
												className="size-4 text-luca-muted"
												fill="none"
												viewBox="0 0 24 24"
												stroke="currentColor"
											>
							...
							<section className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
								...
								<dl className="space-y-3 text-sm">
									...
									<div>
										<dt className="text-xs text-luca-muted mb-0.5">
											Modalidad
										</dt>
										<dd className="font-medium text-luca-muted-dark">
									...
									<div>
										...
										<dd>
											<span
												className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-luca-surface-dark text-luca-muted-dark`}
											>
			...
			>
				<DialogContent className="rounded-2xl bg-white shadow-lg border border-slate-100">
					<DialogHeader>
						<DialogTitle className="text-luca-muted-dark font-heading">
							¿Eliminar cita?
						</DialogTitle>
